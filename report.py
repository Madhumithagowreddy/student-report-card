...
def createdb():
    sq = 'create database studmangement'
    try:
        cursor.execute(sq)
        print('Database called studmangement has been created.')
    except:
...
def display():
    ...
    found = 0
    record = fin.readlines()
    cursor.execute('use studmangement')
...
db = MySQLdb.connect('localhost', 'root', '#2002mad')
cursor = db.cursor()
ch = 1
while ch:
    print()
